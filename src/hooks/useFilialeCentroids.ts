import { useMemo } from 'react';
import type { CommuneData, ProductType, FilialeCentroid } from '../types';
import { computeCentroid } from '../utils/geo';

// Approximate geographic center [lat, lng] for each metropolitan department
const DEPT_CENTERS: Record<string, [number, number]> = {
  '01': [46.07, 5.35],
  '02': [49.47, 3.56],
  '03': [46.39, 3.17],
  '04': [44.10, 6.24],
  '05': [44.66, 6.26],
  '06': [43.87, 7.12],
  '07': [44.75, 4.50],
  '08': [49.62, 4.63],
  '09': [42.90, 1.51],
  '10': [48.30, 4.08],
  '11': [43.10, 2.40],
  '12': [44.28, 2.68],
  '13': [43.54, 5.09],
  '14': [49.09, -0.37],
  '15': [45.03, 2.67],
  '16': [45.72, 0.16],
  '17': [45.86, -0.82],
  '18': [47.02, 2.50],
  '19': [45.35, 1.87],
  '20': [42.15, 9.10],
  '21': [47.42, 4.70],
  '22': [48.44, -2.98],
  '23': [46.07, 2.01],
  '24': [45.14, 0.75],
  '25': [47.16, 6.36],
  '26': [44.68, 5.16],
  '27': [49.08, 1.18],
  '28': [48.32, 1.34],
  '29': [48.27, -4.18],
  '2A': [41.87, 8.98],
  '2B': [42.40, 9.21],
  '30': [43.96, 4.13],
  '31': [43.35, 1.18],
  '32': [43.65, 0.45],
  '33': [44.73, -0.58],
  '34': [43.59, 3.47],
  '35': [48.15, -1.64],
  '36': [46.78, 1.58],
  '37': [47.26, 0.69],
  '38': [45.26, 5.73],
  '39': [46.73, 5.72],
  '40': [43.89, -0.77],
  '41': [47.59, 1.33],
  '42': [45.74, 4.16],
  '43': [45.13, 3.68],
  '44': [47.28, -1.75],
  '45': [47.91, 2.15],
  '46': [44.62, 1.68],
  '47': [44.35, 0.46],
  '48': [44.52, 3.50],
  '49': [47.39, -0.56],
  '50': [48.89, -1.33],
  '51': [48.95, 3.82],
  '52': [48.11, 5.23],
  '53': [48.08, -0.77],
  '54': [48.79, 6.17],
  '55': [49.00, 5.38],
  '56': [47.83, -2.82],
  '57': [49.02, 6.66],
  '58': [47.11, 3.50],
  '59': [50.36, 3.23],
  '60': [49.41, 2.48],
  '61': [48.53, 0.09],
  '62': [50.49, 2.29],
  '63': [45.72, 3.14],
  '64': [43.25, -0.77],
  '65': [43.05, 0.15],
  '66': [42.60, 2.50],
  '67': [48.62, 7.53],
  '68': [47.86, 7.24],
  '69': [45.87, 4.64],
  '70': [47.62, 6.15],
  '71': [46.64, 4.42],
  '72': [47.93, 0.19],
  '73': [45.49, 6.45],
  '74': [46.04, 6.37],
  '75': [48.86, 2.35],
  '76': [49.66, 1.08],
  '77': [48.62, 2.90],
  '78': [48.81, 1.85],
  '79': [46.52, -0.26],
  '80': [49.92, 2.27],
  '81': [43.78, 2.15],
  '82': [44.08, 1.23],
  '83': [43.46, 6.22],
  '84': [44.00, 5.17],
  '85': [46.68, -1.33],
  '86': [46.57, 0.45],
  '87': [45.86, 1.25],
  '88': [48.15, 6.45],
  '89': [47.83, 3.56],
  '90': [47.64, 6.86],
  '91': [48.53, 2.23],
  '92': [48.84, 2.25],
  '93': [48.91, 2.48],
  '94': [48.78, 2.47],
  '95': [49.08, 2.17],
};

const ALL_PRODUCT_TYPES: ProductType[] = ['PP', 'ADBLUE', 'PELLETS_LIV', 'PELLETS_VRAC'];

export function useFilialeCentroids(
  communes: CommuneData[],
  product: ProductType,
  selectedFiliales: string[]
): FilialeCentroid[] {
  return useMemo(() => {
    // Build a map: filiale -> set of departments (for the selected product)
    const filialeDepts = new Map<string, Set<string>>();
    // Build a map: filiale -> count of communes (for the selected product)
    const filialeNbCommunes = new Map<string, number>();

    for (const commune of communes) {
      const filiale = commune.territoires[product];
      if (!filiale || !selectedFiliales.includes(filiale)) continue;

      const dept = commune.departement;
      if (!filialeDepts.has(filiale)) {
        filialeDepts.set(filiale, new Set());
      }
      filialeDepts.get(filiale)!.add(dept);
      filialeNbCommunes.set(filiale, (filialeNbCommunes.get(filiale) ?? 0) + 1);
    }

    // For each filiale, determine which of the 4 product types it covers (across all communes)
    const filialeProduits = new Map<string, Set<ProductType>>();
    for (const commune of communes) {
      for (const pt of ALL_PRODUCT_TYPES) {
        const filiale = commune.territoires[pt];
        if (filiale && filialeDepts.has(filiale)) {
          if (!filialeProduits.has(filiale)) {
            filialeProduits.set(filiale, new Set());
          }
          filialeProduits.get(filiale)!.add(pt);
        }
      }
    }

    // Compute centroids
    const results: FilialeCentroid[] = [];

    for (const [filiale, depts] of filialeDepts) {
      const deptCodes = Array.from(depts).sort();
      const points: [number, number][] = [];

      for (const dept of deptCodes) {
        const center = DEPT_CENTERS[dept];
        if (center) {
          points.push(center);
        }
      }

      const [lat, lng] = computeCentroid(points);

      results.push({
        filiale,
        lat,
        lng,
        nbCommunes: filialeNbCommunes.get(filiale) ?? 0,
        departements: deptCodes,
        produits: Array.from(filialeProduits.get(filiale) ?? []),
      });
    }

    return results.sort((a, b) => b.nbCommunes - a.nbCommunes);
  }, [communes, product, selectedFiliales]);
}
