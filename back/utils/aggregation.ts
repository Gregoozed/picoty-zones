import type { CommuneData, ProductType, DepartmentAggregation, RegionAggregation } from '../types/index.js';
import { referentiel } from '../data/referentiel.js';
...
export function aggregateByDepartment(
  ...
  for (const [codeDept, deptCommunes] of deptMap) {
    ...
    result.push({ codeDept, filialeDominante, filialesPresentes, totalCommunes, communesCouvertes });
  }
...
export function aggregateByRegion(
  ...
  for (const [codeRegion, regionCommunes] of regionMap) {
    ...
    result.push({ codeRegion, filialeDominante, filialesPresentes, totalCommunes, communesCouvertes });
  }
